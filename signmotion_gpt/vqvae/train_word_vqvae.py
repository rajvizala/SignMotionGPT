...
class NpzMotionDataset(Dataset):
    def __init__(self, root_dir, stats_path=None, min_seq_len=64):
        ...
        if stats_path and os.path.exists(stats_path):
            ...
            print("[Warning] [Dataset] WARNING: Stats file not found. Proceeding without normalization. This will affect loss values and model performance.")
            self.mean = 0
            self.std = 1
    ...
    def cleanup_old_checkpoints(self):
        ...
        if len(checkpoints) > self.max_checkpoints:
            ...
            for old_checkpoint in checkpoints[:-self.max_checkpoints]:
                ...
                print(f"[Cleanup] Removed old checkpoint: {old_checkpoint}")
    def load_latest_checkpoint(self):
        checkpoints = glob.glob(os.path.join(self.checkpoint_dir, 'vqvae_epoch_*.pt'))
        if not checkpoints: return None
        latest_checkpoint_path = max(checkpoints, key=os.path.getmtime)
        print(f"[Resume] Loading latest checkpoint: {latest_checkpoint_path}")
...
def initialize_codebook_from_dataset(model, dataloader, num_batches=100):
    print(f"[Init] Collecting data from {num_batches} batches for codebook initialization...")
    all_latents = []
    model.eval()
    with torch.no_grad():
    ...
    print("[OK] Codebook initialized successfully from a diverse data sample.")
    model.train()
...
def train_vqvae_colab(vq_model, dataset, checkpoint_dir, num_epochs=300, batch_size=32, lr=2e-4):
    ...
    if checkpoint:
        ...
        print(f"[OK] Resumed training from epoch {start_epoch}")
